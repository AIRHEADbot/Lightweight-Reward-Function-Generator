...
from generate_reward_DPO import generate_reward_function, validate_reward 
...
def main():
    ...
    reward_func = None
try:
    code = generate_reward_function(save_dir=config.SAVE_DIR)  # 使用训练保存目录
    reward_func = validate_reward(code)
    if reward_func:
        print("成功生成奖励函数")
    else:
        raise RuntimeError("奖励函数验证失败")
